#ifndef KRUSKAL_FUNCS_H
#define KRUSKAL_FUNCS_H
...
//The function that gives the maximum element
int max(int x, int y) {
    if (x > y)
        return x;
    return y;
...
//The function that gives the minimum element
int min(int x, int y) {
    if (x < y)
        return x;
    return y;
